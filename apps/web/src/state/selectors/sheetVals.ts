...
// engine の可変状態(state)を読み、公開 computed(meeting.resolvePos)や bound ハンドラ・
...
export function sheetVals(engine: RoomEngine) {
  ...
  return {
    ...
    pickPrivate: engine.session.pickPrivate,
    pickPublic: engine.session.pickPublic,
    titleVal: s.roomTitle,
    onTitle: (e: ChangeEvent<HTMLInputElement>) => engine.session.onTitleInput(e.target.value),
    warnPublic: s.warnPublic,
    confirmPublic: engine.session.confirmPublic,
    cancelPublic: engine.session.cancelPublic,
